#!/usr/bin/env python3
"""
Script para executar o scraper de borracharias de São Paulo de forma controlada
"""
...
class ScraperController:
    ...
    def run_batch(self, start_cell, batch_size=100):
        """Executa um lote de células"""
        print(f"🚀 Executando lote: células {start_cell} a {start_cell + batch_size - 1}")
        
        cmd = [
            sys.executable, 
            'scraper_borracharias_sp_completo.py', 
            str(start_cell), 
            str(batch_size)
        ]
    ...
    def run_continuous(self, batch_size=100, max_batches=None):
        """Executa o scraper de forma contínua"""
        print("🚀 INICIANDO SCRAPER CONTÍNUO DE BORRACHARIAS - SÃO PAULO")
        print("=" * 70)
        ...
        print(f"📍 Começando da célula: {start_cell:,}")
        ...
        while start_cell < total_cells:
            ...
            print(f"\n📦 LOTE {batch_count}")
            print(f"📍 Células: {start_cell:,} a {start_cell + current_batch_size - 1:,}")
            ...
            if success:
                ...
                if start_cell < total_cells:
                    pause_time = 30  # 30 segundos
                    print(f"⏸️ Pausa de {pause_time}s entre lotes...")
                    time.sleep(pause_time)
            else:
                print("❌ Falha no lote - tentando novamente em 60s...")
                time.sleep(60)
    ...
    def status(self):
        ...
        print("📊 STATUS DO SCRAPER DE BORRACHARIAS - SÃO PAULO")
        print("=" * 60)
        ...
        print(f"⏰ Última atualização: {progress.get('timestamp', 'N/A')}")
        ...
        if cells_processed > 0:
            ...
            print(f"📊 Média por célula: {avg_establishments_per_cell:.2f}")
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Uso:")
        print("  python executar_scraper_sp.py status")
        print("  python executar_scraper_sp.py run [batch_size] [max_batches]")
        return
