#!/usr/bin/env python3
"""
Spider para borracharias - baseado no simple_vpn_spider funcionando
"""
...
class BorrachariaVpnSpider(scrapy.Spider):
    name = 'borracharia_vpn'
    ...
    def start_requests(self):
        ...
        self.logger.info("🔧 BORRACHARIA VPN SPIDER INICIADO - COLETANDO BORRACHARIAS")
        self.logger.info(f"📊 BORRACHARIA_SPIDER_STATS: {{'status': 'started', 'timestamp': '{datetime.now().isoformat()}'}}")
        ...
        self.logger.info(f"📊 BORRACHARIA_SPIDER_STATS: {{'total_cells': {len(cells)}, 'total_requests': {total_requests}, 'search_terms': {len(self.SEARCH_TERMS)}}}")
    ...
    def parse_simple(self, response):
        ...
        for i, card in enumerate(cards):
            try:
                ...
                establishment = {
                    'name': name,
                    'place_id': f"borracharia_{cell_index}_{i}_{hash(name) % 10000}",
                    'rating': rating,
                    'reviews_count': None,
    ...
    def closed(self, reason):
        ...
        self.logger.info(f"🔧 BORRACHARIA VPN SPIDER FINALIZADO - Motivo: {reason}")
        self.logger.info(f"📊 BORRACHARIA_SPIDER_STATS: {{'status': 'finished', 'reason': '{reason}', 'timestamp': '{datetime.now().isoformat()}'}}")
